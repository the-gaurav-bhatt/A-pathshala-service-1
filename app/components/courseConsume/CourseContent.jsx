          ...
          >
            ...
            >
              <h2 className="text-base sm:text-lg font-bold">
                {section.chapterName}: {section.chapterTitle}
              </h2>
              <svg
                className={`ml-2 md:ml-4 h-4 sm:h-6 w-4 sm:w-6 transform ${
                  isSectionExpanded(section._id) ? 'rotate-180' : ''
                }`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
            ...
            {isSectionExpanded(section.id) && (
              <div className="flex flex-col mt-2">
                {section.videoTitles.map((video, index) => (
                  <div key={video._id} className="flex items-center mt-2">
                    <svg
